...
export const generateInvoice = (res, order) => {
    ...
    order.orderedItems.forEach(item => {
        ...
        doc.text(`${item.product.name} (${variant?.color || 'N/A'})`, 50, y);
    ...
    doc.font("Helvetica-Bold")
       ...
       .text(`₹${order.totalPrice.toLocaleString()}`, 480, summaryTop + 10, { width: 70, align: "right" });

    doc.text("GST (18%):", 350, summaryTop + 25)
       .text(`₹${(order.totalPrice * 0.18).toLocaleString()}`, 480, summaryTop + 25, { width: 70, align: "right" });

    if (order.discount > 0) {
        doc.text("Discount:", 350, summaryTop + 40)
           .text(`-₹${order.discount.toLocaleString()}`, 480, summaryTop + 40, { width: 70, align: "right" });
    }

    doc.fontSize(14).fillColor("#0055ff")
       .text("Total Amount:", 350, summaryTop + 60)
       .text(`₹${order.finalAmount.toLocaleString()}`, 480, summaryTop + 60, { width: 70, align: "right" });
